...
async function fetchGeminiJson({ apiKey, model, systemInstruction, userParts, schema, maxOutputTokens, timeout = 28_000 }) {
  ...
  if (!response.ok) throw new Error(clean(payload?.error?.message, 180) || `Gemini HTTP ${response.status}`);
...
export async function analyzePayload(raw = {}, env = process.env) {
  ...
  try {
    ...
    throw appError('Live analysis failed safely. Please retry or use the guided demo.', 502);
  }
}
