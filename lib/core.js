...
const DEFAULT_LIVE_DEADLINE_MS = 25_000;
const DEFAULT_HEDGE_DELAY_MS = 4_000;
const MAX_TEXT = 1800;
...
export function resolveGeminiConfig(env = process.env) {
  ...
  const configuredFallback = normalizeEnvironmentValue(env.GEMINI_FALLBACK_MODEL);
  const fallbackModel = configuredFallback.toLowerCase() === 'none'
    ? ''
    : (configuredFallback || DEFAULT_FALLBACK_MODEL);
  const deadlineMs = Math.round(clamp(env.GEMINI_DEADLINE_MS, 8_000, 27_000, DEFAULT_LIVE_DEADLINE_MS));
  const hedgeDelayMs = Math.round(clamp(env.GEMINI_HEDGE_DELAY_MS, 0, 10_000, DEFAULT_HEDGE_DELAY_MS));
  return {
    apiKey,
    model,
...
function classifyGeminiFailure(error) {
  const diagnostic = redactDiagnostic(error?.diagnostic || error?.message || error?.name || 'Unknown provider failure');
  ...
  wrapped.attempts = error?.attempts || null;
  return wrapped;
}

function linkedAbortSignal(timeout, externalSignal) {
  const controller = new AbortController();
  const timeoutError = new Error('The operation was aborted due to timeout');
  timeoutError.name = 'TimeoutError';
  const timer = setTimeout(() => controller.abort(timeoutError), timeout);
  const forwardAbort = () => controller.abort(externalSignal?.reason || new Error('Request cancelled.'));
...
async function fetchGeminiJson({ apiKey, model, systemInstruction, userParts, schema, maxOutputTokens, timeout = 24_000, signal, thinkingLevel = 'low' }) {
  ...
  const abort = linkedAbortSignal(timeout, signal);
  try {
...
async function callGemini(input, apiKey, model, options = {}) {
...
function retryableGeminiFailure(error) {
  const status = Number(error?.upstreamStatus) || 0;
  if (error?.name === 'TimeoutError' || error?.name === 'AbortError') return true;
  if ([404, 408, 409, 429].includes(status) || status >= 500) return true;
  if (['GEMINI_EMPTY', 'GEMINI_INVALID_JSON', 'GEMINI_INVALID_OUTPUT', 'GEMINI_TRUNCATED'].includes(error?.code)) return true;
  return !status && !['GEMINI_SAFETY'].includes(error?.code);
}

function combineGeminiFailures(models, attempts, deadlineError) {
  const completed = attempts.filter(Boolean);
  const chosen = completed.find(item => !retryableGeminiFailure(item.error))
    || completed.find(item => Number(item.error?.upstreamStatus) >= 500)
    || completed.at(-1);
  const summaries = models.map((model, index) => {
    const error = attempts[index]?.error;
    if (!error) return `${model}: deadline reached while pending`;
    const status = Number(error.upstreamStatus) || null;
    return `${model}: ${error.code || error.name || 'Error'}${status ? ` HTTP ${status}` : ''} — ${redactDiagnostic(error.message)}`;
  });
  const combined = new Error(summaries.join(' | '));
  combined.name = chosen?.error?.name || deadlineError?.name || 'Error';
  combined.code = chosen?.error?.code || 'GEMINI_UNAVAILABLE';
  combined.upstreamStatus = Number(chosen?.error?.upstreamStatus) || null;
  combined.diagnostic = summaries.join(' | ');
  combined.attempts = models.map((model, index) => ({
    model,
    code: attempts[index]?.error?.code || (attempts[index] ? 'ERROR' : 'DEADLINE_PENDING'),
    upstreamStatus: Number(attempts[index]?.error?.upstreamStatus) || null
  }));
  return combined;
}

async function callGeminiResilient(input, apiKey, config) {
  const models = [config.model, config.fallbackModel].filter(Boolean);
  if (models.length === 1) {
    return { live: await callGemini(input, apiKey, models[0], { timeout: config.deadlineMs }), model: models[0], fallbackUsed: false };
  }
  ...
  return new Promise((resolve, reject) => {
    const cleanup = () => {
      ...
      controller.abort(new Error('Another Gemini attempt completed.'));
    };
    const fail = error => {
      if (settled) return;
      settled = true;
    ...
    const maybeFinishFailures = () => {
      if (!settled && attempts[0] && attempts[1]) {
        fail(combineGeminiFailures(models, attempts));
      }
    };
    const attempt = (model, index) => {
      callGemini(input, apiKey, model, { timeout: config.deadlineMs, signal: controller.signal })
        .then(live => {
          if (settled) return;
          settled = true;
          cleanup();
          resolve({ live, model, fallbackUsed: index > 0 });
        })
        .catch(error => {
          if (settled) return;
          attempts[index] = { model, error };
          if (index === 0) {
            if (retryableGeminiFailure(error)) startFallback();
    ...
    hedgeTimer = setTimeout(startFallback, config.hedgeDelayMs);
    deadlineTimer = setTimeout(() => {
      const deadlineError = new Error('The resilient Gemini deadline was reached.');
      deadlineError.name = 'TimeoutError';
      fail(combineGeminiFailures(models, attempts, deadlineError));
    }, config.deadlineMs);
    attempt(models[0], 0);
  });
}
...
export async function analyzePayload(raw = {}, env = process.env) {
  ...
  try {
    const outcome = await callGeminiResilient(input, apiKey, config);
    return {
      id: makeId(),
      createdAt: new Date().toISOString(),
      meta: {
        ...
        warning: outcome.fallbackUsed
          ? `The primary model was unavailable or slow, so ${outcome.model} completed this analysis.`
          : ''
      },
      ...outcome.live
    };
